            ...
            <div className="flex flex-col gap-8 p-6 md:p-8 bg-slate-50 dark:bg-zinc-950 min-h-[calc(100vh-4rem)] transition-colors duration-300">
                <div className="flex items-center justify-between">
                    <div className="flex flex-col gap-2">
                        <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-zinc-100 transition-colors duration-300">Validasi PPh</h1>
                        <p className="text-slate-500 dark:text-zinc-400 transition-colors duration-300">Kelola status validasi PPh dokumen lelang.</p>
                    </div>
                </div>
                        ...
                        <div className="flex w-full md:w-auto items-center gap-3">
                            <form onSubmit={handleSearch} className="relative w-full md:w-64">
                                ...
                                <Input
                                    ...
                                    className="pl-9 w-full shadow-sm rounded-full bg-slate-50 dark:bg-zinc-950 border-slate-200 dark:border-zinc-800 focus-visible:ring-2 focus-visible:ring-emerald-500 ring-offset-white dark:ring-offset-zinc-950 duration-300 transition-all text-slate-900 dark:text-zinc-100"
                                />
                            </form>
                            <Button 
                                onClick={() => setShowAddForm(!showAddForm)} 
                                className="gap-2 shadow-sm hover:shadow-md transition-all duration-300 whitespace-nowrap rounded-full px-5 bg-emerald-600 hover:bg-emerald-700 text-white"
                            >
                        ...
                        <div className="p-6 bg-slate-50/50 dark:bg-zinc-900/50 border-b border-slate-100 dark:border-zinc-800/50 animate-in slide-in-from-top-4 fade-in duration-300">
                            <form onSubmit={handleAddSubmit} className="flex gap-4 items-end max-w-lg">
                                <div className="flex-1 space-y-2">
                                    <label className="text-sm font-semibold text-slate-700 dark:text-zinc-300 transition-colors duration-300">Nomor Validasi Baru (angka saja)</label>
                                    <Input
                                        required
                                        placeholder="Misal: 200"
                                        value={nomorPengajuan}
                                        onChange={(e) => setNomorPengajuan(e.target.value.replace(/[^0-9]/g, ''))}
                                        className="shadow-sm bg-white dark:bg-zinc-950 border-slate-200 dark:border-zinc-800 focus-visible:ring-2 focus-visible:ring-emerald-500 ring-offset-white dark:ring-offset-zinc-950 duration-300 transition-all text-slate-900 dark:text-zinc-100"
                                    />
                                    <p className="text-[10px] text-slate-500 dark:text-zinc-400 transition-colors duration-300">Format: <span className="font-bold text-slate-700 dark:text-zinc-300">{nomorPengajuan ? `${nomorPengajuan}/V-PPh/2026` : '.../V-PPh/2026'}</span></p>
                                </div>
                                <Button 
                                    type="submit" 
                                    disabled={processing} 
                                    className={`rounded-full px-6 bg-emerald-600 hover:bg-emerald-700 text-white transition-all duration-300 ${processing ? 'opacity-50 cursor-not-allowed' : ''}`}
                                >
                            ...
                            <table className="w-full">
                                <thead className="bg-slate-50/80 dark:bg-zinc-900/80 border-b border-slate-100 dark:border-zinc-800/50 transition-colors duration-300">
                                    <tr className="text-left text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">
                                        <th className="px-6 py-5">No. Validasi PPh</th>
                                        ...
                                        <tr>
                                            <td colSpan={3} className="p-12 text-center">
                                                <div className="flex flex-col items-center justify-center space-y-3 text-slate-500 dark:text-zinc-400 transition-colors duration-300">
                                                    ...
                                                    <p className="text-base font-medium">Tidak ada data validasi PPh ditemukan.</p>
                                                </div>
                                            </td>
                                        </tr>
                                ...
                                {documents.links.map((link: any, index: number) => (
                                    link.url ? (
                                        <Button
                                            ...
                                            size="sm"
                                            className={`min-w-9 h-9 px-3 rounded-full transition-all duration-300 font-bold ${link.active ? 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm border-transparent' : 'text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 border-slate-200 dark:border-zinc-700'}`}
