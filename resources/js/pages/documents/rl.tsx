...
export default function DocumentsRLIndex({ documents, filters }: any) {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            case 'tidak_valid': return 'bg-red-50 text-red-900 border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20';
        }
    };
            ...
            <div className="flex flex-col gap-8 p-6 md:p-8 bg-slate-50 dark:bg-zinc-950 min-h-[calc(100vh-4rem)] transition-colors duration-300">
                <div className="flex items-center justify-between">
                    <div className="flex flex-col gap-2">
                        <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-zinc-100 transition-colors duration-300">Dokumen Kutipan RL</h1>
                        <p className="text-slate-500 dark:text-zinc-400 transition-colors duration-300">Kelola status Kutipan Risalah Lelang.</p>
                    </div>
                </div>
                ...
                <Card className="rounded-3xl shadow-sm hover:shadow-md transition-all duration-300 bg-white dark:bg-zinc-900 border-slate-200/60 dark:border-zinc-800/50">
                        ...
                        <div className="flex w-full md:w-auto items-center gap-3">
                            <form onSubmit={handleSearch} className="relative w-full md:w-64">
                                ...
                                <Input
                                    ...
                                    className="pl-9 w-full shadow-sm rounded-full bg-slate-50 dark:bg-zinc-950 border-slate-200 dark:border-zinc-800 focus-visible:ring-2 focus-visible:ring-amber-500 ring-offset-white dark:ring-offset-zinc-950 duration-300 transition-all text-slate-900 dark:text-zinc-100"
                                />
                            </form>
                            <Button 
                                onClick={() => setShowAddForm(!showAddForm)} 
                                className="gap-2 shadow-sm hover:shadow-md transition-all duration-300 whitespace-nowrap rounded-full px-5 bg-amber-600 hover:bg-amber-700 text-white"
                            >
                            ...
                            <form onSubmit={handleAddSubmit} className="flex gap-4 items-end max-w-lg">
                                <div className="flex-1 space-y-2">
                                    ...
                                    <Input
                                        ...
                                        className="shadow-sm bg-white dark:bg-zinc-950 border-slate-200 dark:border-zinc-800 focus-visible:ring-2 focus-visible:ring-amber-500 ring-offset-white dark:ring-offset-zinc-950 duration-300 transition-all text-slate-900 dark:text-zinc-100"
                                    />
                                    <p className="text-[10px] text-slate-500 dark:text-zinc-400 transition-colors duration-300">Format: <span className="font-bold text-slate-700 dark:text-zinc-300">{nomorPengajuan ? `${nomorPengajuan}/K-RL/2026` : '.../K-RL/2026'}</span></p>
                                </div>
                                <Button 
                                    type="submit" 
                                    disabled={processing} 
                                    className={`rounded-full px-6 bg-amber-600 hover:bg-amber-700 text-white transition-all duration-300 ${processing ? 'opacity-50 cursor-not-allowed' : ''}`}
                                >
                    ...
                    <CardContent className="p-0">
                                        ...
                                        <tr>
                                            <td colSpan={3} className="p-12 text-center">
                                                <div className="flex flex-col items-center justify-center space-y-3 text-slate-500 dark:text-zinc-400 transition-colors duration-300">
                                                    ...
                                                    <p className="text-base font-medium">Tidak ada data Kutipan RL ditemukan.</p>
                                                </div>
                                            </td>
                                        </tr>
                                        ...
                                        <tr key={doc.id} className="transition-colors duration-300 hover:bg-slate-50/50 dark:hover:bg-zinc-800/30 group">
                                            ...
                                            <td className="px-6 py-6">
                                                <select
                                                    ...
                                                    className={`text-xs font-bold rounded-full px-4 py-2 appearance-none outline-none border cursor-pointer transition-all duration-300 hover:opacity-80 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-amber-500 ring-offset-white dark:ring-offset-zinc-900 ${getStatusColor(doc.status_proses)}`}
                                                >
                        ...
                        <div className="p-6 border-t border-slate-100 dark:border-zinc-800/50 flex flex-col sm:flex-row items-center justify-between text-sm text-slate-500 dark:text-zinc-400 gap-4 transition-colors duration-300">
                            <span>Menampilkan <span className="font-bold text-slate-900 dark:text-zinc-100">{documents.data.length}</span> dari total <span className="font-bold text-slate-900 dark:text-zinc-100">{documents.total}</span> pengajuan</span>
                                ...
                                {documents.links.map((link: any, index: number) => (
                                    link.url ? (
                                        <Button
                                            ...
                                            size="sm"
                                            className={`min-w-9 h-9 px-3 rounded-full transition-all duration-300 font-bold ${link.active ? 'bg-amber-600 hover:bg-amber-700 text-white shadow-sm border-transparent' : 'text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 border-slate-200 dark:border-zinc-700'}`}
